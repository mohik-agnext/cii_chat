...
def classify_question_type(query):
    ...
    eligibility_patterns = ['eligibility', 'qualify', 'eligible', 'criteria', 'who can', 'requirements']
    process_patterns = ['process', 'procedure', 'how to', 'steps', 'apply', 'application', 'submit']
    fee_patterns = ['fee', 'cost', 'price', 'charge', 'payment', 'expense', 'how much']
    penalty_patterns = ['penalty', 'fine', 'violation', 'punish', 'consequence']
...
def compress_context(query, results, max_length=6000):
    ...
    total_length = sum(len(r.get('metadata', {}).get('text', '')) for r in results)
    ...
    important_patterns = [
        "eligibility criteria", "requirements", "incentives", "benefits", 
        "application process", "fee structure", "penalties", "section", "chapter"
    ]
    ...
    for result in results:
        ...
        for i, sentence in enumerate(sentences):
            ...
            term_match_score = sum(1 for term in query_terms if term in sentence_lower)
            ...
            # 4. Length factor (prefer medium-length sentences between 10-30 words)
            words = len(sentence.split())
            length_score = 1.0 if 10 <= words <= 30 else (0.5 if words < 10 else 0.7)
        ...
        doc_allocation = max(300, max_length // result_count)  # At least 300 chars
        ...
        top_indices = []
        for i, score, length in sentence_scores[:5]:  # Always include top 5 highest scoring
        ...
        compressed_text = '. '.join([sentences[i] for i in all_indices if i < len(sentences)])
        ...
        result_copy['metadata']['compression_ratio'] = f"{len(compressed_text)}/{len(text)}"
...
def rerank_results(query, results, top_k=5):
    ...
    query_lower = query.lower()
    is_specific = any(x in query_lower for x in ["specific", "exact", "precise", "definition", "what is", "what are"])
    is_process = any(x in query_lower for x in ["how to", "process", "steps", "procedure", "submit", "apply"])
    is_eligibility = any(x in query_lower for x in ["eligible", "qualify", "criteria", "requirement", "who can"])
    is_comparison = any(x in query_lower for x in ["compare", "difference", "versus", "vs", "better"])
    
    # Identify entities in query
    entity_patterns = ["MSME", "EV", "electric vehicle", "startup", "business", "industrial", "incentive"]
    query_entities = [entity for entity in entity_patterns if entity.lower() in query_lower]
...
# Progressive search route will be registered after search system initialization
...
def parallel_search(query, top_k=5, alpha=0.5, fusion_method="rrf"):
    ...
    # Check cache first
    cache_key = get_cache_key(query, {"top_k": top_k, "alpha": alpha, "fusion": fusion_method})
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        vector_future = executor.submit(searcher._semantic_search, query, top_k)
    ...
    if fusion_method == "rrf":
        results = searcher._reciprocal_rank_fusion(vector_results, bm25_results)
    elif fusion_method == "combmnz":
        results = searcher._combmnz_fusion(vector_results, bm25_results)
    else:  # weighted
        results = searcher._weighted_fusion(vector_results, bm25_results, alpha)
...
def get_searcher():
    ...
    while searcher is None:
        if time.time() - wait_start > 120:  # 120 second timeout (increased from 30)
            raise Exception("Searcher initialization timeout. Please try again later.")
        time.sleep(0.5)
        # Print progress message every 10 seconds
        if searcher is None and (time.time() - wait_start) % 10 < 0.5:
            print(f"Still waiting for searcher initialization... ({int(time.time() - wait_start)} seconds elapsed)")
            sys.stdout.flush()
...
def save_conversation(session_id, query, answer=None, metadata=None):
    ...
    print(f"Answer: {answer[:50]}..." if answer and len(answer) > 50 else f"Answer: {answer}")
    ...
    if session_history[session_id]:
        ...
        if last_entry.get('user') == query:
            ...
            print(f"Found matching last entry to update for query: {query[:30]}...")
        else:
            ...
            for entry in reversed(session_history[session_id]):
                if entry.get('user') == query:
                    ...
                    print(f"Found matching earlier entry to update for query: {query[:30]}...")
                    break
    
    if existing_entry and answer is not None:
        # Update the existing entry with the new answer
        print(f"Updating existing entry with answer: {answer[:50]}..." if answer and len(answer) > 50 else f"Updating existing entry with answer: {answer}")
        ...
        print(f"Added new conversation exchange with ID: {len(session_history[session_id])}")
        print(f"Exchange details: User query: '{query[:30]}...' Answer: '{answer[:30] if answer else None}...'")
    ...
    for i, entry in enumerate(session_history[session_id]):
        print(f"  {i+1}. User: {entry.get('user', '')[:30]}... Answer: {entry.get('answer', '')[:30] if entry.get('answer') else 'None'}...")
    
    # Debug what was saved
    print(f"Conversation in session {session_id}:")
    print(f"  User query: {query[:50]}..." if len(query) > 50 else f"  User query: {query}")
    print(f"  Answer: {answer[:50]}..." if answer and len(answer) > 50 else f"  Answer: {answer}")
...
def generate_llm_response(query, search_results, session_id, model=None, api_key=None, stream=False, include_history=True):
...
"""
    ...
    try:
        ...
        if stream:
            ...
            # We'll create a wrapper that captures the full response while streaming
            full_response = []
            ...
            def stream_wrapper():
                ...
                try:
                    ...
                    # After streaming is complete, save the full response to conversation history
                    complete_response = ''.join(full_response)
                    print(f"Stream completed. Full response length: {len(complete_response)} chars")
                    print(f"First 100 chars of response: {complete_response[:100]}...")
                    
                    # Always save the conversation with the complete response
                    print(f"Saving complete response to conversation history for session {session_id}")
                    save_conversation(
                        ...
                        metadata={
                            "search_results": [
                                ...
                                } for r in search_results[:3]  # Just save top 3 for storage efficiency
                            ]
                        }
                    )
                except Exception as e:
            ...
            if history and history[-1].get('user') == query:
                ...
                print(f"Updating existing conversation entry for session {session_id}")
                history[-1]['answer'] = answer
            else:
                # Create a new entry if needed
                print(f"Creating new conversation entry for session {session_id}")
...
def perform_recursive_retrieval(query, initial_results, searcher, max_depth=1, max_total_results=20):
    """
    ...
    query_lower = query.lower()
    recursive_keywords = ['eligibility', 'requirements', 'criteria', 'license', 'permit', 
                         'procedure', 'regulation', 'compliance', 'fee', 'penalty']
    
    needs_recursive = any(keyword in query_lower for keyword in recursive_keywords)
    ...
    query_keywords = [word for word in query_lower.split() 
                      if len(word) > 3 and word not in ['what', 'when', 'where', 'which', 'how', 'does', 'that', 'this']]
    ...
    for result in initial_results:
        ...
        if section and any(keyword in section.lower() for keyword in recursive_keywords):
            policy_sections.add(section)
        
        # Extract potential key entities with more targeted approach
        license_matches = re.findall(r'([L]-\d+[A-Z]?|license type [A-Z0-9-]+)', text, re.IGNORECASE)
        policy_matches = re.findall(r'([A-Z][a-z]+ (Policy|Act|Guidelines|Rules|Fee|Requirements))', text)
    ...
    for entity in key_entities:
        ...
        requirement_terms = [term for term in recursive_keywords if term in query_lower]
    ...
    print(f"Generated {len(follow_up_queries)} follow-up queries for recursive retrieval")
    ...
    try:
        ...
        if generate_response:
            ...
            if isinstance(llm_result, dict) and 'answer' in llm_result:
                ...
                llm_response = f"Error generating response: {llm_result['error']}"
            else:
...
def search_stream():
    ...
    if request.method == 'POST':
        data = request.json
        if not data:
            return Response(generate_error_stream("No JSON data provided"), mimetype='text/event-stream')
        query = data.get('query', '')
        session_id = data.get('session_id', '')
        include_history_param = data.get('include_history')
        include_history = True if include_history_param is None else (include_history_param == True or str(include_history_param).lower() == 'true')
        measure_step_times = data.get('measure_step_times', False)
    else:  # GET request
        query = request.args.get('query', '')
        session_id = request.args.get('session_id', '')
        include_history_param = request.args.get('include_history')
        include_history = True if include_history_param is None else (include_history_param.lower() == 'true')
        measure_step_times = request.args.get('measure_step_times', 'false').lower() == 'true'
    
    print(f"Streaming request with query: '{query}', session_id: '{session_id}', include_history: {include_history}")
    ...
    if history:
        ...
        for i, exchange in enumerate(history):
            ...
            user_msg = user_msg[:50] + ('...' if len(user_msg) > 50 else '') if user_msg else ''
            answer = exchange.get('answer', '')
            answer = answer[:50] + ('...' if len(answer) > 50 else '') if answer else ''
    ...
    if measure_step_times:
        timing_metrics['query_preprocessing_time'] = time.time() - preprocess_start
    ...
    if request.method == 'POST':
        ...
        fusion_method = request.args.get('fusion_method', default_fusion_method)
        include_history_param = request.args.get('include_history')
        include_history = True if include_history_param is None else (include_history_param.lower() == 'true')
    ...
    if measure_step_times:
        timing_metrics['conversation_history_time'] = time.time() - history_retrieval_start
    ...
    try:
        ...
        if measure_step_times:
            timing_metrics['query_classification_time'] = time.time() - classify_start
        ...
        if measure_step_times:
            timing_metrics['hybrid_search_time'] = time.time() - vector_search_start
        ...
        if measure_step_times:
            timing_metrics['metadata_filtering_time'] = time.time() - filter_start
        ...
        if measure_step_times:
            timing_metrics['initial_reranking_time'] = time.time() - rerank_start
        
        search_time = time.time() - search_start
        if measure_step_times:
            timing_metrics['total_search_time'] = search_time
            
        print(f"Search completed in {search_time:.2f}s, found {len(reranked_results)} results")
        
        # Use recursive retrieval for policy questions
        if "policy" in query.lower() or query_type in ["eligibility", "process"]:
            recursive_start = time.time()
            combined_results = perform_recursive_retrieval(query, reranked_results, get_searcher())
            recursive_time = time.time() - recursive_start
            print(f"Recursive retrieval completed in {recursive_time:.2f}s, total results: {len(combined_results)}")
            # Re-rank again after recursive retrieval
            final_results = rerank_results(query, combined_results, top_k=top_k)
        else:
        ...
        for i, result in enumerate(final_results):
            ...
            if 'rerank_components' in result:
                context_debug = f"(Score: {score:.4f}, Source: {source}, Exact Match: {result['rerank_components']['exact_match']:.2f})"
                print(f"Result {i+1}: {doc_title[:40]}... {context_debug}")
            else:
                print(f"Result {i+1}: {doc_title[:40]}... (Score: {score:.4f}, Source: {source})")
                        ...
                        "search_results": [
                            {
                                ...
                                "document_title": r.get('metadata', {}).get('document_title', ''),
                                "section": r.get('metadata', {}).get('section', '')
                            } for r in final_results[:3]  # Just save top 3 for storage efficiency
                        ],
...
def search_stream_v2():
    ...
    if not query:
        return Response(generate_error_stream("No query provided"), mimetype='text/event-stream')
    ...
    if is_greeting:
        return Response(generate_greeting_stream(session_id, query), mimetype='text/event-stream')
    ...
    try:
        ...
        return Response(
            generate_stream(query, session_id, final_results, include_history=include_history),
        ...
        import traceback
        traceback.print_exc()
        return Response(generate_error_stream(str(e)), mimetype='text/event-stream')
...
def generate_stream(query, session_id, final_results, include_history=True):
    ...
    history = get_conversation_history(session_id)
    if history:
        print(f"Found {len(history)} previous exchanges in conversation history for session {session_id}")
        for i, exchange in enumerate(history[-3:]):  # Show last 3 exchanges
            print(f"  Exchange {i+1}: User: {exchange.get('user', '')[:30]}... Answer: {exchange.get('answer', '')[:30]}...")
    else:
    ...
    # Note: We don't save conversation here because it's already saved in the stream_wrapper
...
if __name__ == '__main__':
    ...
    try:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            ...
            try:
                ...
                if cmdline and 'python' in proc.info['name'].lower() and 'hybrid_search_server.py' in ' '.join(cmdline):
    ...
    # Run the Flask app
    port = 8080  # Use port 8080 to avoid conflicts with macOS AirPlay (port 5000)
